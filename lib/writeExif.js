    ...
    new Promise( ( resolve , reject ) => {
      fs.readFile(fileObj.outputFilePath , ( err , data ) => {
          if( err ) reject( new Error(err) )
            ...
            // set lens namne (for native lens or cpu lens)
            let inputLensData = []
            if(!buildCamLensInfo.LensName && exif[piexif.ExifIFD.LensModel]){
              for(let lns of fileObj.camLens.lens){
                if((lns.exifName).toLowerCase() == exif[piexif.ExifIFD.LensModel].toLowerCase()){
                  inputLensData.push(lns.exifName)
                  if(!buildCamLensInfo.LensName){
                    buildCamLensInfo.LensName = lns.names[ Math.floor(Math.random() * lns.names.length) ]
                  }else{
                    console.log( "\u001b[33mConflict lens data. Please check the exifName in cameraLens.json : \u001b[0m" + path.basename( fileObj.outputFilePath ) )
                    console.log( inputLensData )
                  }
                }
              }
            }
