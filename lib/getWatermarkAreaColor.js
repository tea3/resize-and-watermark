    ...
    new Promise( ( resolve , reject ) => {
      if( fileObj.config.watermarkColor && fileObj.config.watermarkColor == "auto" ){
        fs.readFile( fileObj.path , ( err , data ) => {
          ...
          let avgG = 0;
          let avgB = 0;
          let quality = (fileObj.config.colorTiefQuality_watermarkArea || 1)
          
          // RGBの平均色を取得
          for(var y = wPos_Y; y < (wPos_Y + resizedWaterMarkSize.height) ; y = y + quality){
              for(var x = wPos_X; x < (wPos_X + resizedWaterMarkSize.width) ; x = x + quality){
